...
tests and attempts.
** Pro:
*** By creating another model `TriviaResult` instead of using `TriviaTest` for recording any past `TriviaTest` records, we can
...
===== The UI feedback when a MatchCommand is executed
* **Alternative 1 (current choice):** Uses `AsyncEventCenter` to dispatch its flash UI events.
** Pro:
