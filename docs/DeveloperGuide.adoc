...
* `ImportFile#parseFileToCards()` -- Parse file in a specific format into a unique list of cards.
...
These operations are exposed in the Model interface as `Model#parseFileToCards()`,
`Model#haveAnyCard()`, `Model#addMultipleCards()` respectively.
...
The sequence diagram below shows how the import of multiple cards.
