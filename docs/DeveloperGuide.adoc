...
* `ImportFile#parseFileToCards()` -- Parses a file in a specific format into a unique list of cards.
...
These operations are exposed in the Model interface as `Model#haveAnyCard()`, `Model#addMultipleCards()` respectively.
...
Step 5: The unique card list is then added into `TriviaBundle`. The number of cards and the cards imported are displayed to the user for verification. +
