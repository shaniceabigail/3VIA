...
The diagram below is an example scenario and how the `MatchTest` behaves at each step.
...
Step 4. Suppose the user chose to match a question to an answer, a list of steps will be executed as shown in the sequence
diagram below.
...
Step 7. At this stage, the System will await `exit` command from the user so that user can be taken back to the test menu page.
