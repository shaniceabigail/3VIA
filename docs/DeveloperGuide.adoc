...
** Pro: Allows the user to undo/ redo an import as the execution of the import only executes `VersionedAddressBook#commit()` once.
...
** Con: Unable to undo/ redo import of cards as the execution of the import command executes `VersionedAddressBook#commit()` multiple times.
