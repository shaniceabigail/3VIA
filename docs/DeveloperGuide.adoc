...
** Cons: Currently, the classes created are for the sole of having static display indexes during `MatchTest`.
* **Alternative 2:** Create a `List` of mapping that maps the actual index to the display index of the question and answer.
** Pros: The implementation details will be contained within `MatchTest`.
** Cons: Additional work is require to maintain the `List` data structure, to ensure that it is updated every time a question or answer is removed.
