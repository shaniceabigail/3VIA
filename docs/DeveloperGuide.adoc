...
** Pro:
*** Less IO intensive as only one write to file is needed.
** Con:
*** May have heap memory issue if large amounts of cards are stored temporarily.
* **Alternative 2:** Writes a card to a xml file after each card is added.
** Pro:
*** Easy to implement.
** Con:
*** May have performance issue when large amount of cards are added.

===== Aspect: How the import file is parsed

* **Alternative 1 (current choice):** Tokenize string according to defined characters between question and answer, and between cards.
** Pro:
*** Allows user to undo/ redo import as import is viewed by `VersionedAddressBook` as a single command.
** Con:
*** May have memory performance issue as multiple cards are stored temporarily in the `TriviaBundle`.
* **Alternative 2:** Tokenize string based on AddCommand input format.
** Pro:
*** Easy to implement as it allows reuse of add command functions.
** Con:
*** Unable to undo/ redo import of all cards as the import command is essentially multiple add commands.
...
there are 2 test features in 3VIA, namely, the Open-Ended Test and the Match Test.
...
<<Design-Model>>. For Storage, we have `TriviaResultsStorage`, as shown in <<Design-Storage>>.
For UI, we have `TriviaTestPage` and `TriviaTestResultPage`, as shown in <<Design-Ui>>.
...
** Pros:
*** Follows Open-Close Principle which increases the extensibility of the testing feature in 3VIA.
*** Increases the re-usability of similar codes, and promotes cleaner code.
** Con:
*** The implementation of different tests have to follow the structure of TriviaTest very strictly.
* **Alternative 2:** Implement each kinds of test as a standalone test with its own standalone class and event handlers.
** Pro:
*** Easy to implement and each developer will have their freedom of how to implement their own test.
** Con:
*** Will lead to code that is increasingly not manageable.

===== The Model of TriviaTest's Results
* **Alternative 1 (current choice):** Create another Model called `TriviaResult` to initialize the past TriviaTest's
tests and attempts.
** Pro:
*** By creating another class called `TriviaResult` which will be used to record any past `TriviaTest` records, we can
reduce the amount of data collected. Hence, improving the start up time of the application.
* **Alternative 2:** Use `TriviaTest` itself to initialize the past TriviaTest's tests and attempts.
** Pro:
*** Since `TriviaResult` is a subset `TriviaTest`, it is possible to remove the need of `TriviaResult`, thereby reducing the
coupling of the application's Model component.
** Con:
*** However, rebuilding a new but completed `TriviaTest` would be tedious and complicated. Hence, `TriviaResult` is
needed separate the complexities that are involved in data collection and the actual life cycle of `TriviaTest`.
// end::triviatest[]

// tag::matchtest[]
=== MatchTest Feature
==== Current Implementation
The life cycle of a MatchTest is facilitated by the `AppState` and `MatchTest` model.
...
Step 2. The `MatchingTestCommand` will be executed and a new MatchTest Object will be created as shown in the below Sequence diagram.
...
** Pro:
*** Finding the exact question and answer is easy as what the user see is a representative of the Id that identifies the question/answer.
** Con:
*** Currently, the classes created are for the sole purpose of having static display indexes during `MatchTest`.
* **Alternative 2:** Create a `List` of mapping that maps the actual index to the display index of the question and answer.
** Cons:
*** Additional work is required to maintain the `List` data structure, to ensure that it is updated every time a question or answer is removed from the UI.
*** The questions and answers are displayed using `ListView`, and since the mapping of actual index to display index is not an attribute to the question/answer,
...
create a static display index of question and answer. Hence, this design consideration was not implemented due to abstraction that is leaking into the UI.
...
** Pro:
*** The feedback received by the user is more synchronised between question and answer.
** Con:
*** More CPU intensive as it uses multi-threading.
* **Alternative 2:** Uses `EventCenter` to dispatch its UI event.
** Pro:
*** Easier to debug code that does not use multi-threading and less CPU intensive.
** Con:
*** The events will be dispatched and handle in a sequential order, as a result the UI feedback for question may not be in sync with feedback for answer.
// end::matchtest[]
