...
** Cons: The questions and answers are displayed using `ListView`, and since the mapping of actual index to display index is not an attribute to the question/answer,
...
create a static display index of question and answer. Hence, this design consideration was scrapped due to abstraction that is leaking into the UI.
...
** Pros: The feedback received by the user is more synchronised between question and answer.
...
** Cons: The events will be dispatched and handle in a sequential order, as a result the UI feedback for question may
not be in sync with feedback for answer.
