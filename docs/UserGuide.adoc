...
_Imports the contents from the file.txt text file from the specified file location._
****

[NOTE]
The file path refers to the absolute file path of the import text file.

.3 simple steps to kickstart your amazing learning experience with 3VIA!: +
. Create an import file. +

image::import-file-format.png[caption = "download link"]
[IMPORTANT]
Only .txt files are allowed.

[options = "header", cols = "1,2", caption=""]
.Import file format legend
|===
|Keys| Purpose
|kbd:[SPACE] + kbd:[t] + kbd:[/]| To be placed before each topic
|kbd:[TAB]| To separate a question and an answer
|kbd:[&#9166;]| To separate each card
|===
[start = 2]
. Copy the absolute file path of the text file. +
. Import to 3VIA! +
insert 3VIA UI

[WARNING]
Duplicate questions will result in import failing.
